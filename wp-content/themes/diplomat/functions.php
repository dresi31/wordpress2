...
if ( !function_exists('tmm_comments_form_defaults') ) {
	function tmm_comments_form_defaults($defaults) {
		...
		$defaults['fields']['author'] = '<div class="row"><div class="large-7 columns"><fieldset class="comment-form-author">' .
			'<input id="author" name="author" type="text" value="' . esc_attr($commenter['comment_author']) . '" size="30"' . $aria_req . ' placeholder="' . esc_html__('Your Name', 'diplomat') . ' *" /></fieldset>';
		$defaults['fields']['email'] = '<fieldset class="comment-form-email">' .
			'<input id="email" name="email" type="email" value="' . esc_attr($commenter['comment_author_email']) . '" size="30"' . $aria_req . ' placeholder="' . esc_html__('Your Email', 'diplomat') . ' *" /></fieldset>';
		$defaults['fields']['url'] = '<fieldset class="comment-form-url"> ' .
			'<input id="url" name="url" type="url" value="' . esc_attr($commenter['comment_author_url']) . '" size="30" placeholder="' . esc_html__('Website', 'diplomat') . '" /></fieldset></div></div>';
		$defaults['comment_field'] = '<fieldset class="comment-form-comment">' .
			'<textarea cols="30" rows="10" required="" id="comment" name="comment" placeholder="' . esc_html__('Your Message', 'diplomat') . ' *"></textarea></fieldset>';

		$defaults['comment_notes_before'] = '';
		$defaults['comment_notes_after'] = '';

		$defaults['cancel_reply_link'] = ' - ' . esc_html__('Cancel reply', 'diplomat');

		$defaults['title_reply'] = esc_html__('Leave a Reply', 'diplomat');

		$defaults['label_submit'] = esc_html__('Submit', 'diplomat');
                ...
                <article>
                    ...
                    <div class="comment-body">
                        ...
                        <div class="comment-meta">
                            ...
                            <div class="comment-date"><?php comment_date(); ?> <?php esc_html_e('at', 'diplomat'); ?> <?php comment_date('H:i'); ?></div>
                            <?php echo get_comment_reply_link(array_merge(
                                    array('reply_text' => esc_html__('Reply', 'diplomat')),
	...
	function tmm_admin_notices() {
		...
		if (!is_writable(TMM_THEME_PATH . "/css/styles.css") && !get_option('tmm_dismiss_style-css-notice')) {
			$notices .= sprintf(__('<div class="notice error style-css-notice is-dismissible"><p>'
				. '<b>' . esc_html__('Notice:', 'diplomat') . ' </b>'
				. esc_html__('permissions 755 (in some cases 775) for %s/css/styles.css file are required for correct theme work.', 'diplomat') . '<br>'
				. esc_html__('Please follow', 'diplomat') . ' <a href="' . TMM_SITE_LINK . 'tutorials/permissions/" target="_blank">'
				. esc_html__('this link', 'diplomat') . '</a> ' . esc_html__('to read the instructions how to do it properly.', 'diplomat')
		...
		if (!is_writable(TMM_THEME_PATH . "/scss/_tmm_styling_options.scss") && !get_option('tmm_dismiss_styling-options-notice')) {
			$notices .= sprintf(__('<div class="notice error styling-options-notice is-dismissible"><p>'
				. '<b>' . esc_html__('Notice:', 'diplomat') . ' </b>'
				. esc_html__('permissions 755 (in some cases 775) for %s/scss/_tmm_styling_options.scss file are required for correct theme work.', 'diplomat') . '<br>'
				. esc_html__('Please follow', 'diplomat') . ' <a href="' . TMM_SITE_LINK . 'tutorials/permissions/" target="_blank">'
				. esc_html__('this link', 'diplomat') . '</a> ' . esc_html__('to read the instructions how to do it properly.', 'diplomat')
				. '</p></div>', 'diplomat'), TMM_THEME_PATH);
		}
		...
		if (!is_writable(TMM_THEME_PATH . "/css/custom2.css") && !get_option('tmm_dismiss_custom-css2-notice')) {
			$notices .= sprintf(__('<div class="notice error custom-css2-notice is-dismissible"><p>'
				. '<b>' . esc_html__('Notice:', 'diplomat') . ' </b>'
				. esc_html__('permissions 755 (in some cases 775) for %s/css/custom2.css file are required for correct theme work.', 'diplomat') . '<br>'
				. esc_html__('Please follow', 'diplomat') . ' <a href="'.TMM_SITE_LINK.'tutorials/permissions/" target="_blank">'
				. esc_html__('this link', 'diplomat') . '</a> ' . esc_html__('to read the instructions how to do it properly.', 'diplomat')
				. '</p></div>', 'diplomat'), TMM_THEME_PATH);
		}
		...
		if ( (!class_exists('TMM_Theme_Features') || !class_exists('TMM_Content_Composer')) && !get_option('tmm_dismiss_required-plugins-notice') ) {
			$notices .= sprintf(__('<div class="error notice required-plugins-notice is-dismissible"><p>'
				. '<b>' . esc_html__('Notice:', 'diplomat') . ' </b>'
				. esc_html__('For correct theme work you need to install ThemeMakers Required Plugins.', 'diplomat') . '<br>'
				. esc_html__('Please follow', 'diplomat') . ' <a href="' . admin_url('themes.php?page=tgmpa-install-plugins') . '">'
				. esc_html__('this link', 'diplomat') . '</a> ' . esc_html__('to proceed the installation.', 'diplomat')
...
if ( !function_exists('tmm_read_more_link') ) {
	function tmm_read_more_link() {
		return '<a class="button default tag_more" href="' . esc_url(get_permalink()) . '">' . esc_html__('Read More', 'diplomat') . '</a>';
	}
}
...
if ( !function_exists('tmm_after_setup_theme') ) {
	function tmm_after_setup_theme(){
		...
		register_nav_menu('primary', esc_html__('Primary Menu', 'diplomat'));
...
if ( !function_exists('tmm_admin_enqueue_scripts') ) {
	function tmm_admin_enqueue_scripts() {
		...
		$translation_array = array(
			...
			'edit' => esc_html__('Edit', 'diplomat'),
			'delete' => esc_html__('Delete', 'diplomat'),
		...
		return array(
			...
			*/
			...
			array(
				...
				'slug'               => 'tmm_db_migrate',
				'source'             => 'https://github.com/ThemeMakers/tmm_db_migrate/archive/diplomat_v2.0.5.zip',
				'required'           => false,
				'version'           => '2.0.5',
			),
			array(
				...
				'source'             => 'https://github.com/ThemeMakers/tmm_content_composer/archive/diplomat_v1.1.2.zip',
				'required'           => true,
				'version'           => '1.1.2',
			),
			array(
			...
			array(
				...
				'source'             => 'https://downloads.wordpress.org/plugin/seamless-donations.4.0.23.zip',
				'required'           => false,
				'version'           => '4.0.23',
			),
			array(
				'name'              => esc_html__( 'Envato Market', 'diplomat' ),
				'slug'              => 'envato-market',
				'source'            => 'https://github.com/envato/wp-envato-market/releases/download/2.0.0/envato-market-v2.0.0.zip',
				'required'          => false,
				'version'           => '2.0.1'
			),
			array(
			...
			array(
				...
				'slug'               => 'LayerSlider',
				'source'             => TMM_THEME_URI . '/install/layersliderwp-6.8.2.zip',
				'required'           => false,
				'version'           => '6.8.2',
			),
			array(
				'name'                  => esc_html__( 'Classic Editor', 'diplomat' ),
				'slug'                  => 'classic-editor',
